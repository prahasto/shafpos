...
class Pos extends MY_Controller {
	...
	function addretur(){
		...
		$data = array(
                ...
                'total' => $this->tec->formatDecimal($npaid),
    ...
    function getSalesbyID(){
        $saleid = $this->input->get('salesid');
        $obj = new stdClass();
        $items =$this->pos_model->getSaleItemsbySaleID($saleid);
        krsort($items);
      ...
      //  $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items);
     //  echo json_encode( $this->pos_model->getSaleItemsbySaleID($saleid) );
...
//'$salesno' => $salesno,
            ...
            if (!$eid && !$suspend && $paid) {
				...
				$myJSON = json_decode($arr);
				foreach ($myJSON as $k => $v) {
				   //$id = $myJSON->$k->reffnumber;
				   $npayingby_id = $myJSON->$k->paidbyID;
		           $npayingby_name =$myJSON->$k->paidby;
				   $namount = $myJSON->$k->amount;
				   ...
				   $payment[] = array(
						...
						'amount' => $namount,
						'customer_id' => $customer_id,
						'paid_by' =>$npayingby_name,
						'paid_by_id' =>$npayingby_id,
						'bank_id' =>$nbankid,
						'edc_id' =>$nedcid,
						'reference' => $nreff,
						'created_by' => $this->session->userdata('user_id'),
...
//alert("add sales"); payment
    ...
    function autocompletesalesno() {
        ...
        $rows = $this->pos_model->getSalesNo($term);
        if ($rows) {
            foreach ($rows as $row) {
                $cust[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id,
                    'name' => $row->salesno,
